...
export class CardsUtils {
    ...
    computeValue(a: number){
        return (a % numberOfCardsPerPlayer) < 4 ? 0 : ((a % numberOfCardsPerPlayer) == 7 ? 3 : 1);
    }
    ...
    findHighestCardBySeed(trick: number[], suit: number){
      const maxNumber: number = Math.max(...trick.filter(c => this.computeSeed(c) == suit));
      return trick.indexOf(maxNumber);
    }

    //check if a user has a seed
    isSeedOnHand(userId: number, deck: number[], suit: number, is_played: boolean[]) {
        for (let i = 0; i < numberOfCardsPerPlayer; i++) {
          var actSeed = this.computeSeed(deck[userId * numberOfCardsPerPlayer + i]);
    ...
    winCard(firstCard: number, secondCard: number, trump: number) {
        if (this.computeSeed(firstCard) == this.computeSeed(secondCard) && (firstCard < secondCard)) return true;
        if (this.computeSeed(firstCard) != trump && this.computeSeed(secondCard) == trump) return true;
        return false;
    }
    ...
    let ris = [];
    for (let i = 0; i < 4; i++) {
      let sorted = deck.slice(i * numberOfCardsPerPlayer, (i + 1) * numberOfCardsPerPlayer).sort((a, b) => a - b);
